 ...
 * 4. 支持重试机制（指数退避）
 * 5. 支持 Beta 功能（context-management）
 * 6. 自动调整 thinking 参数
 * 7. 返回 Anthropic 的响应给客户端
 */
...
function adjustThinkingParams(body: any): any {
	if (body.thinking && typeof body.thinking === 'object') {
		...
		if (maxTokens && budgetTokens && budgetTokens > maxTokens) {
			...
			console.log(`[/v1/messages] 检测到 budget_tokens (${budgetTokens}) > max_tokens (${maxTokens})，自动调整 max_tokens 为 ${newMaxTokens}`);
			return {
				...body,
...
export async function POST(req: Request) {
	try {
		...
		if (envModel) {
			console.log(`[/v1/messages] 使用环境变量 model: ${envModel} (原始 model: ${body.model})`);
			body.model = envModel;
		}
		...
		if (betaHeader) {
			...
			console.log('[/v1/messages] 使用 Beta 功能:', betaHeader);
		}
		...
		console.error('[/v1/messages] 错误类型:', err.constructor?.name);
		console.error('[/v1/messages] 错误信息:', err.message);
		console.error('[/v1/messages] 错误堆栈:', err.stack);
...
async function handleNonStreamingRequest(
	...
	for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
		try {
			...
			console.log('[/v1/messages] 耗时:', duration, 'ms');
			...
			if (!isRetryableError(error)) {
				console.error('[/v1/messages] 不可重试的错误，直接返回');
				break;
			}
			...
			if (attempt < MAX_RETRIES) {
				...
				const err = error as Error;
				console.warn(`[/v1/messages] 第 ${attempt + 1} 次重试失败: ${err.message}, ${retryDelay.toFixed(0)}ms 后重试`);
				await delay(retryDelay);
			} else {
				console.error(`[/v1/messages] 重试 ${MAX_RETRIES} 次后仍失败`);
			}
		}
	}
	...
	if (axios.isAxiosError(lastError) && lastError.response) {
		console.error('[/v1/messages] Axios 错误状态码:', lastError.response.status);
		console.error('[/v1/messages] Axios 错误响应体:', JSON.stringify(lastError.response.data, null, 2));
		console.error('[/v1/messages] ========== 错误结束 ==========');
...
async function handleStreamingRequest(
	...
	for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
		try {
			...
			// 判断是否可重试
			if (!isRetryableError(error)) {
				console.error('[/v1/messages] 流式请求不可重试的错误，直接返回');
				throw error;
			}
			...
			if (attempt < MAX_RETRIES) {
				const retryDelay = calculateRetryDelay(attempt);
				const err = error as Error;
				console.warn(`[/v1/messages] 流式请求第 ${attempt + 1} 次重试失败: ${err.message}, ${retryDelay.toFixed(0)}ms 后重试`);
				await delay(retryDelay);
			} else {
				console.error(`[/v1/messages] 流式请求重试 ${MAX_RETRIES} 次后仍失败`);
				throw lastError;
			}
		}
	}
	...
	const readable = new ReadableStream({
			...
			try {
				response.data.on('data', (chunk: Buffer) => {
					...
					try {
						...
						// Debug 模式：记录所有数据块
						if (isDebugMode) {
							console.log('[/v1/messages] 流式数据块 #', chunkCount, ':', chunk.toString());
						} else {
							// 非 Debug 模式：只记录统计信息（每 10 个块）
							if (chunkCount % 10 === 0) {
								console.log(`[/v1/messages] 已传输 ${chunkCount} 个数据块, ${byteCount} 字节`);
							}
						}

						controller.enqueue(chunk);
					} catch (error) {
						console.error('[/v1/messages] 处理数据块错误:', error);
				...
				response.data.on('end', () => {
					if (!clientDisconnected) {
						...
						console.log(`[/v1/messages] 总计传输: ${chunkCount} 个数据块, ${byteCount} 字节`);
						console.log('[/v1/messages] ========== 请求结束 ==========');
						controller.close();
					}
				});
				...
				response.data.on('error', (error: Error) => {
					...
					if (error.message.includes('aborted') || (error as NodeJS.ErrnoException).code === 'ECONNRESET') {
						console.warn('[/v1/messages] 客户端断开连接（正常）');
						clientDisconnected = true;
					} else {
				...
				response.data.on('close', () => {
					if (!clientDisconnected) {
						console.log('[/v1/messages] 连接关闭');
						clientDisconnected = true;
					}
				});

			} catch (error) {
				console.error('[/v1/messages] 流式响应初始化错误:', error);
				console.error('[/v1/messages] ========== 错误结束 ==========');
				try {
					controller.error(error);
				} catch (e) {
		...
		cancel() {
			// 客户端取消流式传输
			console.log('[/v1/messages] 客户端取消流式传输');
			try {

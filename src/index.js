...
function isRecoveryLateContinuationCandidate(pick = {}, options = {}) {
  ...
  const selectedDayScore = Number(options.selectedDayScore ?? getSelectedBucketDayScore(pick) ?? 0);
  const combinedScore = Number(options.combinedScore ?? getEffectiveCombinedScore(pick, options.runtimeStrategy || {}) ?? 0);
  const historyScore = Number(options.historyScore ?? pick.historyScore ?? 0);
  const signalStrength = Number(options.signalStrength || 0);
  const intradayReturnPct = pick.intradayReturnPct;
  const nowMinutes = options.nowMinutes;

  if (tradeWindows.allowRecoveryLateContinuationEntries === false) return false;
  if (pick.strategy?.bucket !== 'continuation') return false;
  if ((options.confidence || 'LOW') !== 'HIGH') return false;
  if (selectedDayScore < (tradeWindows.recoveryLateContinuationMinDayScore ?? 96)) return false;
  if (historyScore < (tradeWindows.recoveryLateContinuationMinHistoryScore ?? 72)) return false;
  if (combinedScore < (tradeWindows.recoveryLateContinuationMinCombinedScore ?? 84)) return false;
  if (signalStrength < (tradeWindows.recoveryLateContinuationMinSignalStrength ?? 5)) return false;
  if (
    intradayReturnPct != null &&
    intradayReturnPct > (tradeWindows.recoveryLateContinuationMaxIntradayReturnPct ?? 1.35)
  ) return false;
  if (
    volumeRatio < (tradeWindows.recoveryLateContinuationMinVolumeRatio ?? 0.95) ||
    volumeRatio > (tradeWindows.recoveryLateContinuationMaxVolumeRatio ?? 2.4)
  ) return false;
  if (
    history.gain5d == null ||
    history.gain5d < (tradeWindows.recoveryLateContinuationMinGain5d ?? 0.5) ||
    history.gain5d > (tradeWindows.recoveryLateContinuationMaxGain5d ?? 7)
  ) return false;
  if (
    history.rsi == null ||
    history.rsi < (tradeWindows.recoveryLateContinuationMinRsi ?? 46) ||
    history.rsi > (tradeWindows.recoveryLateContinuationMaxRsi ?? 62)
  ) return false;
  if (
    history.macdHistogram != null &&
    history.macdHistogram < (tradeWindows.recoveryLateContinuationMinMacdHistogram ?? -0.2) &&
    history.macdHistogramImproving !== true
  ) return false;
  if (
    Number.isFinite(nowMinutes) &&
    nowMinutes > (tradeWindows.recoveryLateContinuationCutoffMinutes ?? (14 * 60 + 59))
  ) return false;
  return true;
}
...
function setManualTradeStatus(message,type){const el=document.getElementById('manualTradeStatus');if(!el)return;el.textContent=message||'';el.className='trade-status'+(type?(' '+type):'');}
function updateLookupCard(item){const emptyEl=document.getElementById('lookupEmpty');const resultEl=document.getElementById('lookupResult');const buyBtn=document.getElementById('manualBuyBtn');if(!item){currentLookup=null;emptyEl.style.display='block';resultEl.style.display='none';buyBtn.disabled=true;return;}currentLookup=item;emptyEl.style.display='none';resultEl.style.display='block';buyBtn.disabled=!(Number(item.price)>0);document.getElementById('lookupSymbol').textContent=item.symbol||'-';document.getElementById('lookupName').textContent=item.name||'-';document.getElementById('lookupPrice').textContent=item.price!=null?Number(item.price).toFixed(3):'-';document.getElementById('lookupChange').textContent=item.changePercent!=null?((item.changePercent>=0?'+':'')+Number(item.changePercent).toFixed(2)+'%'):'-';document.getElementById('lookupSector').textContent=item.sector||'-';document.getElementById('lookupScore').textContent=item.combinedScore!=null?Number(item.combinedScore).toFixed(2):(item.score!=null?String(item.score):'-');document.getElementById('lookupSuggestion').textContent=item.suggestionText||'-';document.getElementById('lookupReason').textContent=item.suggestionReason||'-';document.getElementById('lookupBucket').textContent=item.bucketLabel?('当前桶：'+item.bucketLabel):'当前桶：-';document.getElementById('lookupEastmoney').href=item.eastmoneyUrl||'#';}
async function lookupSymbol(){const input=document.getElementById('manualSymbolInput');const raw=(input.value||'').trim();if(!raw){setManualTradeStatus('请输入股票代码','error');updateLookupCard(null);return;}setManualTradeStatus('正在查询...');try{const resp=await fetch('/scan/'+encodeURIComponent(raw));const item=await resp.json();if(!resp.ok||!item||!item.symbol){throw new Error(item&&item.error?item.error:'未找到该股票，确认它已在当前扫描池中');}const portfolioResp=await fetch('/portfolio').then(r=>r.json()).catch(()=>null);const cash=portfolioResp&&typeof portfolioResp.cash==='number'?portfolioResp.cash:null;const suggestedAmount=item.manualSuggestion&&typeof item.manualSuggestion.suggestedAmount==='number'?item.manualSuggestion.suggestedAmount:0;item.suggestionText=suggestedAmount>0?(suggestedAmount/10000).toFixed(2)+'万 / '+(item.manualSuggestion?.confidence||'-'):((item.manualSuggestion?.confidence||'-')+' / 0万');item.suggestionReason=item.manualSuggestion?.reason||'无';if(cash!=null){item.suggestionText+=' · 现金'+(cash/10000).toFixed(2)+'万';}item.bucketLabel=item.strategy?.bucketLabel||item.strategy?.bucket||'-';updateLookupCard(item);setManualTradeStatus('查询成功，可直接手动买入。','success');}catch(err){updateLookupCard(null);setManualTradeStatus('查询失败：'+err.message,'error');}}
function clearLookup(){document.getElementById('manualSymbolInput').value='';document.getElementById('manualAmountInput').value='';updateLookupCard(null);setManualTradeStatus('输入代码后可查询当前扫描行情，并直接手动买入。');}
async function manualBuyFromLookup(){if(!currentLookup){setManualTradeStatus('请先查询股票代码','error');return;}const amountInput=document.getElementById('manualAmountInput');const amount=parseFloat((amountInput.value||'').trim());if(Number.isNaN(amount)||amount<=0){setManualTradeStatus('请输入有效的买入金额（万元）','error');return;}const price=Number(currentLookup.price||0);if(!(price>0)){setManualTradeStatus('当前价格无效，无法下单','error');return;}const amountInYuan=amount*10000;const estimatedQty=Math.floor(amountInYuan/price/100)*100;if(estimatedQty<100){setManualTradeStatus('金额不足买入一手','error');return;}const confirmText='手动买入 '+currentLookup.symbol+' '+(currentLookup.name||'')+'\\n价格: '+price.toFixed(3)+'\\n金额: '+amount.toFixed(2)+' 万\\n预计数量: '+estimatedQty+' 股\\n\\n确认提交?';if(!confirm(confirmText)) return;setManualTradeStatus('正在提交买单...');try{const resp=await fetch('/buy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({symbol:currentLookup.symbol,name:currentLookup.name,price:price,amount:amountInYuan,force:true})});const result=await resp.json();if(result.success){setManualTradeStatus('买入成功：'+result.message,'success');setTimeout(()=>location.reload(),800);}else{throw new Error(result.error||'买入失败');}}catch(err){setManualTradeStatus('买入失败：'+err.message,'error');}}
...
class PaperAccount {
  ...
  syncHistoryCache(historyCache = null) {
    if (historyCache instanceof Map) {
      this.historyCache = historyCache;
      return;
    }
    if (!fs.existsSync(this.historyCachePath)) return;
    try {
      const data = JSON.parse(fs.readFileSync(this.historyCachePath, 'utf8'));
      this.historyCache = new Map(Object.entries(data));
    } catch (err) {
      console.error('[PAPER] 加载历史缓存失败:', err.message);
    }
  }
  ...
  getAdaptiveConfig() {
    ...
    return {
      ...
      tradeWindows: {
        ...
        allowRecoveryLateContinuationEntries: tradeWindows.allowRecoveryLateContinuationEntries ?? true,
        recoveryLateContinuationCutoffMinutes: tradeWindows.recoveryLateContinuationCutoffMinutes ?? (14 * 60 + 59),
        recoveryLateContinuationMinDayScore: tradeWindows.recoveryLateContinuationMinDayScore ?? 96,
        recoveryLateContinuationMinHistoryScore: tradeWindows.recoveryLateContinuationMinHistoryScore ?? 72,
        recoveryLateContinuationMinCombinedScore: tradeWindows.recoveryLateContinuationMinCombinedScore ?? 84,
        recoveryLateContinuationMinSignalStrength: tradeWindows.recoveryLateContinuationMinSignalStrength ?? 5,
        recoveryLateContinuationMinVolumeRatio: tradeWindows.recoveryLateContinuationMinVolumeRatio ?? 0.95,
        recoveryLateContinuationMaxVolumeRatio: tradeWindows.recoveryLateContinuationMaxVolumeRatio ?? 2.4,
        recoveryLateContinuationMinGain5d: tradeWindows.recoveryLateContinuationMinGain5d ?? 0.5,
        recoveryLateContinuationMaxGain5d: tradeWindows.recoveryLateContinuationMaxGain5d ?? 7,
        recoveryLateContinuationMaxIntradayReturnPct: tradeWindows.recoveryLateContinuationMaxIntradayReturnPct ?? 1.35,
        recoveryLateContinuationMinRsi: tradeWindows.recoveryLateContinuationMinRsi ?? 46,
        recoveryLateContinuationMaxRsi: tradeWindows.recoveryLateContinuationMaxRsi ?? 62,
        recoveryLateContinuationMinMacdHistogram: tradeWindows.recoveryLateContinuationMinMacdHistogram ?? -0.2,
      },
      recoveryGuard: {
      ...
      positionSizing: {
        ...
        secondaryContinuationMultiplier: adaptive.positionSizing?.secondaryContinuationMultiplier ?? 0.85,
        recoveryModeMultiplier: adaptive.positionSizing?.recoveryModeMultiplier ?? 0.55,
        guardedRecoveryMultiplier: adaptive.positionSizing?.guardedRecoveryMultiplier ?? 0.45,
        drawdownPressureMultiplier: adaptive.positionSizing?.drawdownPressureMultiplier ?? 0.9,
        maxDrawdownMultiplier: adaptive.positionSizing?.maxDrawdownMultiplier ?? 0.7,
        highContinuationMinPositionValue: adaptive.positionSizing?.highContinuationMinPositionValue ?? 350000,
      ...
      positionManagement: {
        ...
        continuationAddOnMaxPnlPct: adaptive.positionManagement?.continuationAddOnMaxPnlPct ?? 12,
      ...
      mainEntryGuard: {
        ...
        highMinSignalStrength: adaptive.mainEntryGuard?.highMinSignalStrength ?? 4,
        mediumMinSignalStrength: adaptive.mainEntryGuard?.mediumMinSignalStrength ?? 3,
        highMinDayScore: adaptive.mainEntryGuard?.highMinDayScore ?? 78,
        mediumMinDayScore: adaptive.mainEntryGuard?.mediumMinDayScore ?? 70,
        minGain10d: adaptive.mainEntryGuard?.minGain10d ?? -2.5,
        maxGain10d: adaptive.mainEntryGuard?.maxGain10d ?? 6,
        minGain5d: adaptive.mainEntryGuard?.minGain5d ?? -4.5,
        highMaxGain5d: adaptive.mainEntryGuard?.highMaxGain5d ?? 1.8,
        mediumMaxGain5d: adaptive.mainEntryGuard?.mediumMaxGain5d ?? 2.5,
        minDistanceToHigh60d: adaptive.mainEntryGuard?.minDistanceToHigh60d ?? -22,
        maxDistanceToHigh60d: adaptive.mainEntryGuard?.maxDistanceToHigh60d ?? -6,
        minDeviationFromMA20: adaptive.mainEntryGuard?.minDeviationFromMA20 ?? -3,
        maxDeviationFromMA20: adaptive.mainEntryGuard?.maxDeviationFromMA20 ?? 2.2,
        minVolumeRatio: adaptive.mainEntryGuard?.minVolumeRatio ?? 0.9,
        highMaxVolumeRatio: adaptive.mainEntryGuard?.highMaxVolumeRatio ?? 1.35,
        mediumMaxVolumeRatio: adaptive.mainEntryGuard?.mediumMaxVolumeRatio ?? 1.45,
        highMaxIntradayReturnPct: adaptive.mainEntryGuard?.highMaxIntradayReturnPct ?? 1.8,
        mediumMaxIntradayReturnPct: adaptive.mainEntryGuard?.mediumMaxIntradayReturnPct ?? 2.5,
        maxPullbackFromHighPct: adaptive.mainEntryGuard?.maxPullbackFromHighPct ?? 1,
        maxOpenDrawdownPct: adaptive.mainEntryGuard?.maxOpenDrawdownPct ?? 1.5,
      },
      overnightRisk: {
  ...
  getPositionSizingContext(marketRegime = 'NEUTRAL') {
    ...
    const portfolioDrawdown = this.peakEquity > 0
      ? ((this.peakEquity - currentEquity) / this.peakEquity) * 100
      : 0;
    const recoveryGuard = adaptive.recoveryGuard || {};
  ...
  calculateTargetPositionValue(pick, context = {}) {
    const adaptive = context.adaptive || this.getAdaptiveConfig();
    const performanceFeedback = context.performanceFeedback || this.getPerformanceFeedback({ marketRegime: context.marketRegime });
    const marketRegime = context.marketRegime || 'NEUTRAL';
    const confidence = context.confidence || 'LOW';
    const allowGuardedRecovery = context.allowGuardedRecovery === true;
    const recoveryMode = context.recoveryMode === true;
    ...
    const regimeConfig = context.regimeConfig || adaptive.regimeMultipliers[marketRegime] || adaptive.regimeMultipliers.NEUTRAL;
    ...
    if (pick.strategy?.bucket === 'continuation') {
      basePositionValue *= (positionSizing.continuationMultiplier ?? 0.55);
      if (pick.strategy?.selectedReason?.continuationSecondary) {
        basePositionValue *= (positionSizing.secondaryContinuationMultiplier ?? 0.85);
      }
    }
    if (recoveryMode) {
      basePositionValue *= (positionSizing.recoveryModeMultiplier ?? 0.55);
    } else if (allowGuardedRecovery) {
      basePositionValue *= (positionSizing.guardedRecoveryMultiplier ?? 0.45);
    }
    ...
    if (performanceFeedback.drawdownPressure > 3) {
      basePositionValue *= (positionSizing.drawdownPressureMultiplier ?? 0.9);
    }
    if (maxDrawdown > 20) {
      basePositionValue *= (positionSizing.maxDrawdownMultiplier ?? 0.7);
    }

    let minFloor = 0;
    if (confidence === 'HIGH' && pick.strategy?.bucket === 'continuation') {
      minFloor = Math.max(minFloor, positionSizing.highContinuationMinPositionValue ?? 350000);
    }
    if (recoveryMode) {
      minFloor = Math.max(minFloor, positionSizing.recoveryModeMinPositionValue ?? 300000);
    } else if (allowGuardedRecovery) {
      minFloor = Math.max(minFloor, positionSizing.guardedRecoveryMinPositionValue ?? 300000);
    }

    if (minFloor > 0) {
      const cappedFloor = Math.min(minFloor, this.config.maxPositionValue);
      basePositionValue = Math.max(basePositionValue, cappedFloor);
    }
  ...
  buildPositionSignalMap(allMarketData = []) {
    ...
    const candidates = allMarketData
      ...
      .map(item => this.buildTrackedPositionSignal(item))
      .filter(Boolean);
    ...
    return new Map(candidates.map(item => [item.symbol, item]));
  }

  buildTrackedPositionSignal(item = null) {
    if (!item) return null;

    const [candidate] = mergeHistoryIntoMarketItems([{
      ...item,
    ...
    const classified = classifyCandidateBuckets([candidate], this.runtimeStrategy);
    return classified.enriched?.[0] || null;
  }
  ...
  assessAddOnDecision(pos, pick, context = {}) {
    ...
    const addOnMaxPnlPct = snapshot.liveBucket === 'continuation'
      ? (pm.continuationAddOnMaxPnlPct ?? pm.addOnMaxPnlPct ?? 6)
      : (pm.addOnMaxPnlPct ?? 6);
    if ((pos.pnlPct || 0) > addOnMaxPnlPct) return { shouldAdd: false, reason: `浮盈过高不追(${(pos.pnlPct || 0).toFixed(2)}%)` };
  ...
  assessReduceDecision(pos, exitDecision = {}, pick = null, context = {}) {
    ...
    const exitReasons = Array.isArray(exitDecision.reasons) ? exitDecision.reasons : [];
    const isContinuationPosition = (pos.entryBucket || 'main') === 'continuation';
    ...
    const liveBucketLabel = pick?.strategy?.bucketLabel || BUCKET_LABELS[snapshot.liveBucket] || snapshot.liveBucket;
    const continuationWeakReason = snapshot.liveBucket !== 'continuation'
      ? `实时信号降为${liveBucketLabel}`
      : exitReasons[0]
        ? `${exitReasons[0].type}${exitReasons[0].detail ? `(${exitReasons[0].detail})` : ''}`
        : null;
    const trimReason = isContinuationPosition
      ? (
          profitReached && continuationWeakReason
            ? `延续转弱减仓(${continuationWeakReason})`
            : null
        )
      : (
          strongProfitReached
  ...
  assessBuyConfidence(pick, marketRegime, performanceFeedback, options = {}) {
    ...
    let effectiveCombinedScore = combinedScore;
    ...
    if (pick.strategy?.bucket === 'continuation') {
      effectiveCombinedScore = getEffectiveCombinedScore(pick, this.runtimeStrategy);
    ...
    if (mainEntryGuard.enabled && pick.strategy?.bucket !== 'continuation') {
      if (pick.strategy?.bucket !== 'main') {
        return {
          confidence: 'REJECT',
          reason: `普通低吸仅允许主池开仓，当前为${pick.strategy?.bucketLabel || pick.strategy?.bucket || '未知'}`,
        };
      }
      ...
      const mainMinSignalStrength = baseConfidence === 'HIGH'
        ...
        : (mainEntryGuard.mediumMinSignalStrength ?? 3);
      const mainMinDayScore = baseConfidence === 'HIGH'
        ? (mainEntryGuard.highMinDayScore ?? 78)
        : (mainEntryGuard.mediumMinDayScore ?? 70);
      const mainMaxGain5d = baseConfidence === 'HIGH'
        ? (mainEntryGuard.highMaxGain5d ?? 1.8)
        : (mainEntryGuard.mediumMaxGain5d ?? 2.5);
      const mainMaxVolumeRatio = baseConfidence === 'HIGH'
        ? (mainEntryGuard.highMaxVolumeRatio ?? 1.35)
        : (mainEntryGuard.mediumMaxVolumeRatio ?? 1.45);
      const mainMaxIntradayReturnPct = baseConfidence === 'HIGH'
        ? (mainEntryGuard.highMaxIntradayReturnPct ?? 1.8)
      ...
      const mainChecks = {
        ...
        volumeRatio: volumeRatio >= (mainEntryGuard.minVolumeRatio ?? 0.9) && volumeRatio <= mainMaxVolumeRatio,
      };
      const failedMainChecks = Object.entries(mainChecks)
        .filter(([, passed]) => !passed)
        .map(([key]) => key);
      if (failedMainChecks.length > 0) {
        const mainGain10dMin = mainEntryGuard.minGain10d ?? -2.5;
        const mainGain10dMax = mainEntryGuard.maxGain10d ?? 6;
        const mainGain5dMin = mainEntryGuard.minGain5d ?? -4.5;
        const mainDistanceMin = mainEntryGuard.minDistanceToHigh60d ?? -22;
        const mainDistanceMax = mainEntryGuard.maxDistanceToHigh60d ?? -6;
        const mainDeviationMin = mainEntryGuard.minDeviationFromMA20 ?? -3;
        const mainDeviationMax = mainEntryGuard.maxDeviationFromMA20 ?? 2.2;
        ...
        const detailedChecks = formatFailedChecks(failedMainChecks, {
          ...
          gain10d: {
            ...
            op: h.gain10d != null && h.gain10d < mainGain10dMin ? '<' : '>',
          },
          gain5d: {
            value: h.gain5d,
            threshold: h.gain5d != null && h.gain5d < mainGain5dMin ? mainGain5dMin : mainMaxGain5d,
            op: h.gain5d != null && h.gain5d < mainGain5dMin ? '<' : '>',
          },
          distanceToHigh60d: {
            value: h.distanceToHigh60d,
            threshold: h.distanceToHigh60d != null && h.distanceToHigh60d < mainDistanceMin ? mainDistanceMin : mainDistanceMax,
            op: h.distanceToHigh60d != null && h.distanceToHigh60d < mainDistanceMin ? '<' : '>',
          },
          deviationFromMA20: {
            value: h.deviationFromMA20,
          ...
          volumeRatio: {
            value: volumeRatio,
            threshold: volumeRatio < mainVolumeMin ? mainVolumeMin : mainMaxVolumeRatio,
            op: volumeRatio < mainVolumeMin ? '<' : '>',
          },
        }, {
          dayScore: '主池日分',
          historyScore: '历史分',
        ...
        return { confidence: 'REJECT', reason: `主池低吸确认不足(${detailedChecks.join('/')})` };
      }
    }

    if (baseConfidence === 'MEDIUM') {
      const stricterMediumChecks = {
        signalStrength: signalStrength.length >= 2,
        macdHistogram: h.macdHistogram != null && (h.macdHistogram >= -0.05 || h.macdHistogramImproving === true),
        gain5d: h.gain5d != null && h.gain5d >= -6.5 && h.gain5d <= 4,
        deviationFromMA20: h.deviationFromMA20 != null && h.deviationFromMA20 >= -5 && h.deviationFromMA20 <= 3.5,
        volumeRatio: volumeRatio >= 0.85 && volumeRatio <= 1.6,
      };
    ...
    const overnightRiskMinScore = pick.strategy?.bucket === 'continuation'
      ...
      : Math.min(adaptive.overnightRisk.minScoreToCheck ?? 80, 74);
    if (adaptive.overnightRisk?.enabled && combinedScore >= overnightRiskMinScore) {
      const overnightRiskCfg = pick.strategy?.bucket === 'continuation'
        ...
        : {
            ...adaptive.overnightRisk,
            maxGain10d: Math.min(adaptive.overnightRisk.maxGain10d ?? 18, 12),
            maxGain5d: Math.min(adaptive.overnightRisk.maxGain5d ?? 4, 2.5),
            maxDeviationFromMA20: Math.min(adaptive.overnightRisk.maxDeviationFromMA20 ?? 8, 5.5),
            maxVolatility: Math.min(adaptive.overnightRisk.maxVolatility ?? 20, 16),
            maxRecent10LimitUps: 0,
          };
    ...
    if (adaptive.entryQuality?.enabled) {
      ...
      const guardedContinuationEntry =
        continuationEntryQuality &&
        options.allowGuardedRecovery === true &&
        baseConfidence === 'HIGH' &&
        selectedDayScore >= 96 &&
        effectiveCombinedScore >= 82;
      const entryQualityCfg = continuationEntryQuality
        ? {
            ...
            maxPullbackFromHighPct: isAfternoonSession
              ...
              : Math.max(adaptive.entryQuality.maxPullbackFromHighPct ?? 1.2, guardedContinuationEntry ? 3.2 : 2.2),
            maxOpenDrawdownPct: Math.max(adaptive.entryQuality.maxOpenDrawdownPct ?? 1.8, guardedContinuationEntry ? 3.0 : 2.6),
          }
        : adaptive.entryQuality;
  ...
  calculateExitUrgency(pos, pick, marketRegime) {
    ...
    if (pick) {
      ...
      let weakVsOpenNow = false;
      let weakVsPrevCloseNow = false;
      let negativePnl = false;
      let continuationScoreGap = 0;
      let continuationSoftWeakness = false;
      let continuationHardWeakness = false;

      if (isContinuationPosition) {
        const prevClose = Number(pick.prevClose || 0);
        weakVsOpenNow = pick.open ? pos.currentPrice < pick.open * 0.997 : false;
        weakVsPrevCloseNow = prevClose > 0 ? pos.currentPrice < prevClose * 0.998 : false;
        negativePnl = (pos.pnlPct || 0) < 0;
        continuationScoreGap = Math.max(0, exitScoreThreshold - effectiveDayScore);
        const failWeaknessCount = [weakVsOpenNow, weakVsPrevCloseNow, negativePnl].filter(Boolean).length;
        continuationSoftWeakness = failWeaknessCount >= 1 || continuationScoreGap >= 3;
        continuationHardWeakness = failWeaknessCount >= 2 || continuationScoreGap >= 6;
      ...
      if (scoreDrop) {
        let scoreDropWeight = weights.scoreDrop;
        let shouldCountScoreDrop = true;
        if (isContinuationPosition) {
          const protectedContinuationScore =
            entrySelectedDayScore >= 96 &&
            (pos.combinedScore || pos.entryScore || 0) >= 82 &&
            (pos.pnlPct || 0) >= 0;
          if (protectedContinuationScore && !weakVsOpenNow && !weakVsPrevCloseNow && continuationScoreGap <= 35) {
            ...
            scoreDropWeight = (pos.pnlPct || 0) >= 1.5 && !continuationHardWeakness ? 6 : 10;
          } else if (continuationScoreGap <= 4) {
      ...
      if (isContinuationPosition) {
        ...
        const continuationDrawdownDanger = (
          (highestPnlPct >= 2.5 && drawdownFromHigh >= 2.8) ||
          (highestPnlPct >= 4.5 && drawdownFromHigh >= 2.1)
        );
        const continuationDrawdownWeak = (
          weakVsOpenNow ||
          weakVsPrevCloseNow ||
          continuationScoreGap >= 3 ||
          (pos.pnlPct || 0) <= 1.2
        );
        if (continuationDrawdownDanger && continuationDrawdownWeak) {
          const drawdownWeight = highestPnlPct >= 4.5 ? 62 : 48;
          totalUrgency += drawdownWeight;
          reasons.push({ type: '强势回撤过快', weight: drawdownWeight, detail: `最高${highestPnlPct.toFixed(2)}%回撤${drawdownFromHigh.toFixed(2)}%` });
        }
      ...
      } else if (pos.holdDays >= 1 && pick.open && pick.prevClose) {
        const prevClose = Number(pick.prevClose || 0);
        const weakNextDayCarry = (
          prevClose > 0 &&
          pos.currentPrice < pick.open * 0.994 &&
          pos.currentPrice < prevClose * 0.992 &&
          (pos.pnlPct || 0) < -1.2
        );
        if (weakNextDayCarry) {
          totalUrgency += 90;
          reasons.push({ type: '次日承接失效', weight: 90, detail: `现价${pos.currentPrice.toFixed(2)}弱于开盘${pick.open}和昨收${prevClose}` });
        }

        const failedGapHold = (
          prevClose > 0 &&
          pick.open >= prevClose * 1.01 &&
          pos.currentPrice <= prevClose * 0.997
        );
        if (failedGapHold) {
          totalUrgency += 85;
          reasons.push({ type: '高开低走失效', weight: 85, detail: `高开后跌破昨收${prevClose}` });
        }

        if ((pos.holdDays || 0) === 1 && highestPnlPct < 0.8 && (pos.pnlPct || 0) <= -2.4) {
          totalUrgency += 75;
          reasons.push({ type: '隔夜弱修复', weight: 75, detail: `次日未修复且亏损${(pos.pnlPct || 0).toFixed(2)}%` });
        }
      }
    }
  ...
  runTradeCycle(strategyPicks, ts, marketRegime = 'UNKNOWN', allMarketData = []) {
    ...
    const sizingContext = this.getPositionSizingContext(marketRegime);
    const currentEquity = sizingContext.currentEquity;
    const portfolioDrawdown = sizingContext.portfolioDrawdown;
    const recoveryMode = sizingContext.recoveryMode;
    const regimeConfig = sizingContext.regimeConfig;
    const dynamicMaxPositions = sizingContext.dynamicMaxPositions;
    ...
    if (!marketOpen) {
      for (const [symbol, pos] of this.positions.entries()) {
        ...
        this.updatePositionExitSnapshot(pos, {
          urgency: 0,
          shouldExit: false,
          reasons: [{ type: '非交易时间', weight: 0, detail: '仅更新信号快照' }],
        }, pick, currentTime);
      }
    ...
    const recoveryAddOnLimit = sizingContext.recoveryAddOnLimit;
    const protectedRecoveryPositions = Array.from(this.positions.values()).filter(isProfitProtectedPosition);
    const recoveryAnchorPositions = Array.from(this.positions.values()).filter(isRecoveryAnchorPosition);
    ...
    const allowGuardedRecovery = sizingContext.allowGuardedRecovery;
      ...
      const tradeReadyCandidates = strategyPicks
        .filter(p => {
          ...
          if (lastSellTs) {
            ...
            const rapidReentryCandidate =
              p.strategy?.bucket === 'continuation' &&
              (p.tradeDecision?.confidence === 'HIGH' || getSelectedBucketDayScore(p) >= 96) &&
              getEffectiveCombinedScore(p, this.runtimeStrategy) >= 82;
            const effectiveCooldownMinutes = rapidReentryCandidate
              ? Math.min(cooldownMinutes, 20)
              : cooldownMinutes;
            if (minutesSinceSell < effectiveCooldownMinutes) {
          ...
          if (recoveryMode) {
            ...
            const recoveryLateContinuationException = isRecoveryLateContinuationCandidate(p, {
              adaptive,
              runtimeStrategy: this.runtimeStrategy,
              confidence: confidenceDecision.confidence,
              signalStrength,
              combinedScore: getEffectiveCombinedScore(p, this.runtimeStrategy),
              selectedDayScore,
              historyScore: p.historyScore || 0,
              nowMinutes: getBeijingMinutes(currentTime),
            });
            ...
            if (h.gain5d != null && h.gain5d > 6 && !recoveryLateContinuationException) {
    ...
    if (availablePositions <= 0 && addOnCandidates.length === 0) {
      ...
      skippedReason = this.positions.size > 0
        ? `组合回撤超过5%，防守加仓上限${recoveryAddOnLimit}只或无高质量锚仓`
        : '组合回撤超过5%，暂停开仓';
  ...
  getSuggestedPositionValue(pick, marketRegime = 'NEUTRAL') {
    ...
    const basePositionValue = this.calculateTargetPositionValue(pick, {
      adaptive,
      performanceFeedback,
      marketRegime,
      regimeConfig: sizingContext.regimeConfig,
      confidence,
      recoveryMode: sizingContext.recoveryMode,
      allowGuardedRecovery: sizingContext.allowGuardedRecovery,
    });
...
async function main() {
  ...
  if (fs.existsSync(statePath)) {
    try {
      ...
      console.warn(`[STATE] 读取历史状态失败: ${err.message}`);
    }
  }
  const state = {
    startedAt: new Date().toISOString(),
    mode: 'eastmoney-dom-scanner',
    market: persistedState?.market || [],
    strategyPicks: persistedState?.strategyPicks || [],
    observationPicks: persistedState?.observationPicks || [],
    marketCount: persistedState?.marketCount || 0,
    scanRounds: persistedState?.scanRounds || 0,
    lastScanAt: persistedState?.lastScanAt || null,
    marketRegime: persistedState?.marketRegime,
    researchWatchlist: persistedState?.researchWatchlist || { count: 0, generatedFrom: null },
    diagnostics: {
      ...createEmptyScanDiagnostics(),
  ...
  const scanner = new MarketScanner(config, async (payload) => {
    ...
    if (paperAccount && paperAccount.positions.size > 0) {
      paperAccount.syncHistoryCache(scanner.historyCache);
  ...
  const server = http.createServer((req, res) => {
    ...
    if (match) {
      ...
      const item = state.market.find((x) => x.symbol === symbol) || null;
      if (!item) {
        res.writeHead(404, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: 'not_found' }));
        return;
      }
      const suggestion = paperAccount
        ? paperAccount.getSuggestedPositionValue(item, state.marketRegime?.regime || 'NEUTRAL')
        : null;
      const position = paperAccount?.positions?.get?.(symbol) || null;
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({
        ...item,
        manualSuggestion: suggestion,
        inPosition: !!position,
